...
fit_ORC <- function(data, cluster, id, inip, Cov, CovTime, status, covariate,SurvTime,A0, hess, randslope) 
{
...
if (randslope == F){
  predicted_Cov <- function (atf) {
    ...
    fit = lmer(zij ~ start + (1|id) + (1|cluster), data = Dat, control = lmerControl(optimizer = "bobyqa"))
  ...
  t2<-nlminb(inip,Lik_ORC,hessian=hess, control = list(rel.tol = 1e-6) )
  ...
  if (hess == T){
    ...
    std3 <- sqrt(diag(solve(hessian( Lik_ORC, t2$par))))
...
if (randslope == T) {
  ...
  predicted_Cov <- function(atf) {
    ...
    fit <- lmer(zij ~ start+(1|id)+(0+start|id)+(1|cluster), data=Dat,REML = FALSE, control = lmerControl(optimizer = "bobyqa") )
  ...
  t2<-nlminb(inip,Lik_ORC,hessian=hess, control = list(rel.tol = 1e-8) )
