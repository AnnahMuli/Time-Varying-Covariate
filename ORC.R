...
#tt <- sort(unique(Dat$ATF[status==1]  )) # Unique event times
...
predicted_BMD <- function (atf) {
  ...
  fit = lmer(zij ~ start + (1|id) + (1|pair), data = Dat)
...
LMM_Output<-tt[1:nt]|>purrr::map_dfr(predicted_BMD) 
head(LMM_Output,10) 
...
#Obtaining the MLE's
initial2<-c(  log(0.1), log(1.5),0.3,-3.5)
